...
impl<'a> HdrName<'a> {
    ...
    pub fn from_static<F, U>(hdr: &'static [u8], f: F) -> U
    where
        F: FnOnce(HdrName<'_>) -> U,
    {
        let mut buf = uninit_u8_array();
        let hdr = parse_hdr(hdr, &mut buf, &HEADER_CHARS).expect("static str is invalid");
        f(hdr)
    }
}
...
impl<'a> PartialEq<HdrName<'a>> for HeaderName {
    ...
    fn eq(&self, other: &HdrName<'a>) -> bool {
        match self.inner {
            Repr::Standard(s) => match other.inner {
                Repr::Standard(b) => s == b,
                _ => false,
            },
...
impl<'a> Hash for MaybeLower<'a> {
    ...
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        if self.lower {
            ...
            for &b in self.buf {
                state.write(&[HEADER_CHARS[b as usize]]);
            }
        }
    }
}
